<template>
  <div>
    <div class="page-category" data-log="商品分类">
      <div class="page-category-wrap">
        <div class="type-header">
          <div class="left">
            <a class="home">
              <img src="../../static/img/icon_back_n.png" class="ib">
            </a>
          </div>
          <div class="tit">
            <h2 data-log="HEAD-标题-商品分类">
              <span class="title">商品分类</span>
            </h2>
          </div>
          <div class="right">
            <a href="javascript:;">
              <div class="icon iconfont">&#xe62a;</div>
            </a>
          </div>
        </div>
        <div class="list-wrap">
          <div class="js-list-item category0">
            <div class="f-list">
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>手机</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="" src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>电视</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/efb682b76a29411fd26d990971992c3e.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>电脑</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0d7f8167e14f0c70fd84aa6cb321cb6.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>小米手机</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/0ed187521ff28e6616798c1c990c1788.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>红米手机</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/e55a5383-9b7f-1dfd-7f12-e648f69f5c41.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>配件及其他</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>电视</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>音响</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>配件与会员</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>电脑</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>配件及其他</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>家电</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>路由</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>智能</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>电源</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>耳机</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>音箱</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>礼品</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>生活</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>电话卡</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div class="cells_auto_fill">
                  <div data-index="0" class="body" style="width: 5rem; height: 2rem;">
                    <div class="items J_linksign-customize" style="width: 5rem; height: 2rem;">
                      <a class="exposure">
                        <div class="img">
                          <img class="lazy" data-src="" src="../../static/img/2017062113331213369.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section">
                <div name="m1" class="category_title">
                  <span>零售店</span>
                </div>
              </div>
              <div class="section">
                <div class="category_group">
                  <div class="box">
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                    <div class="product">
                      <a class="exposure"
                         data-log_code="31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041"
                         href="/#/product/view?product_id=10000041" data-stat-id="025ab48d5607e4e1"
                         onclick="_msq.push(['trackEvent', '221a86c5a4688b35-025ab48d5607e4e1', '/#/product/viewproduct_id=10000041', 'pcpid', '31wapcategory_newcategory_group001016#rid=02b26b7342d70ef2045f5072ae9a6ccb&amp;t=normal&amp;page=category_new&amp;act=product&amp;pid=10000041']);">
                        <div class="img">
                          <img class="big lazy" data-src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               src="../../static/img/f0e283f74845939fd24d53f7fb1d35c1.png"
                               style="transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);">
                        </div>
                        <div class="name">小米6</div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="list-navbar" :style="{top:top + 'rem'}">
        <ul>
          <li v-for="(v,i) in list_nav" @click="addClass(i)" :class="i==iNow?'active':''"><span>{{v}}</span></li>
        </ul>
      </div>
    </div>
    <div class="footer">
      <div class="fix-footer">
        <ul>
          <li>
            <a href="javascript:;">
              <router-link to="/home"><div class="home"></div>首页</router-link>
            </a>
          </li>
          <li>
            <a href="javascript:;">
              <router-link to="/type"><div class="type"></div>分类</router-link>
            </a>
          </li>
          <li>
            <a href="javascript:;">
              <router-link to="/buycar"><div class="buycar"></div>购物车</router-link>
            </a>
          </li>
          <li>
            <a href="javascript:;">
              <router-link to="/mine"><div class="my"></div>我的</router-link>
            </a>
          </li>
        </ul>
      </div>
    </div>
    </div>
</template>
<script>
//  import router from '../router'
  import $ from 'jquery'
  export default{
    data () {
      return {
        s: '',
        top: '',
        iNow: 0,
        list_nav: ['新品', '手机', '电视', '电脑', '家电', '路由', '智能', '电源', '耳机', '音箱', '礼品', '生活', '米粉卡', '零售店']
      }
    },
    mounted () {
      this.scroll()
      let _this = this
      $('.page-category .page-category-wrap').on('scroll', function () {
        _this.scroll()
      })
    },
    methods: {
      scroll () {
        this.s = $('.page-category .page-category-wrap').scrollTop()
        if (this.s === 0) {
          this.top = 0.84
        }
        if (this.s >= 89) {
          this.top = 0
        }
      },
      addClass (index) {
        this.iNow = index
      }
//      changeR () {
//        router.beforeEach((to, from, next) => {
//          console.log(to, from, next)
//          if (from.name !== 'Home') {
//            to('/home')
//          }
//        })
//      }
    }
  }
</script>
<style>
  /*.page-category .list-navbar::-webkit-scrollbar{display: none;}*/
  .page-category .page-category-wrap {
    position: absolute;
    top: 0;
    bottom: 0;
    left: 0;
    right: 0;
    overflow-y: auto;
    background: #fff;
    padding-bottom: 1.2rem;
  }

  .type-header {
    display: box;
    display: -webkit-box;
    height: .84rem;
    box-align: center;
    -webkit-box-align: center;
    padding: 0 .2rem;
    background: #f2f2f2;
    position: relative;
    border-bottom: 1px solid #eee;
  }

  .type-header .left .ib {
    width: .44rem;
    height: .44rem;
  }

  .type-header .tit {
    -webkit-box-flex: 1;
    box-flex: 1;
    width: 100%;
  }
  .type-header .right a {
    display: inline-block;
    margin-left: .2rem;
    width: .44rem;
    height: .44rem;
  }
  .type-header .right a .iconfont{
    font-size: .44rem;
  }
  .type-header .right .icon {
    color: #999;
  }
  .type-header .tit h2 {
    text-align: center;
    position: absolute;
    top: 0;
    right: 0;
    bottom: 0;
    left: 0;
    line-height: .84rem;
  }

  .type-header .tit span {
    font-size: .3rem;
    color: #666;
    font-weight: 400;
  }

  .page-category .list-wrap {
    margin-left: 1.52rem;
    padding: 2px .32rem;
    overflow: hidden;
  }

  .page-category .list-navbar {
    position: fixed;
    top: .84rem;
    bottom: .8rem;
    width: 1.52rem;
    margin-top: 1px;
    z-index: 90;
    border-right: 1px solid #efefef;
    overflow: hidden;
  }

  .page-category .list-navbar > ul {
    z-index: 90;
    position: absolute;
    top: 0;
    bottom: 0;
    left: 0;
    right: -.3rem;
    padding: .2rem .3rem .2rem 0;
    background: #fefefe;
  }

  .page-category .list-navbar > ul li {
    font-size: .34rem;
    height: .9rem;
    line-height: .9rem;
    overflow: hidden;
    text-align: center;
    -webkit-tap-highlight-color: rgba(0, 0, 0, 0);
    transition: transform .1s linear;
    transform-origin: center center;
    transform: scale(0.76);
    -webkit-transition: -webkit-transform .1s linear;
    -webkit-transform-origin: center center;
    -webkit-transform: scale(0.76);
  }

  .page-category .list-navbar > ul li.active {
    color: #fb7d34;
    transform: scale(1);
    -webkit-transform: scale(1);
  }

  .f-list .cells_auto_fill .items img {
    width: 100%;
  }

  .f-list .category_group .product .img img {
    width: 100%;
  }

  .f-list .category_title {
    background: #fff;
    font-size: .28rem;
    text-align: center;
    font-weight: 400;
    margin-top: .2rem;
    height: 1.28rem;
    line-height: 1.28rem;
    overflow: hidden;
  }

  .f-list .category_title span {
    position: relative;
  }

  .f-list .category_title span::before, .f-list .category_title span::after {
    content: "";
    position: absolute;
    top: 50%;
    left: 0;
    width: .4rem;
    border-top: 1px solid #e0e0e0;
    transform: translate3d(-150%, -50%, 0);
    -webkit-transform: translate3d(-150%, -50%, 0);
  }

  .f-list .category_title span::after {
    left: auto;
    right: 0;
    transform: translate3d(150%, -50%, 0);
    -webkit-transform: translate3d(150%, -50%, 0);
  }

  .f-list .category_group .product {
    float: left;
    width: 33.33333%;
    text-align: center;
    margin-top: .2rem;
    margin-bottom: .3rem;
    overflow: hidden;
  }

  .f-list .category_group .product .img {
    width: 1rem;
    height: 1rem;
    margin: 0 auto;
    background: #FFF;
    overflow: hidden;
  }

  .f-list .category_group .product .name {
    margin-top: .28rem;
    white-space: nowrap;
    font-size: .23rem;
    color: rgba(0, 0, 0, .54);
  }

  .f-list .name {
    font-size: .28rem;
    color: rgba(0, 0, 0, .87);
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
  }

  .f-list .category_group .box {
    width: 100%;
    overflow: hidden;
  }
</style>
